...
UNZ+1+UKHIGHVALUE'"""
        ...
        src_account = paymul.CRAccount(number=12345678,
                                     holder='BAC NET TEST',
                                     currency='CRC',
                                     sortcode=400515)

        dest_account = paymul.CRAccount(number=87654321,
        ...
        batch = paymul.Batch(exec_date=datetime.date(2004, 11, 12),
                             reference='CRHIGHVALUE',
                             debit_account=src_account,
                             name_address="BAC BANK PLC\nHSBC NET TEST\nTEST\nTEST\nCOSTA RICA")
        batch.transactions.append(transaction)

        message = paymul.Message(reference='CRHIGHVALUE',
...
UNZ+1+EZONE'"""
    ...
    def test_cr_low_value_ach_instruction_level(self):
        dest_account1 = paymul.CRAccount(number=87654321,
                                     holder="BAC NET RPS TEST\nBAC BANK",
                                     currency='CRC',
                                     sortcode=403124)
        name_address = "BAC BANK PLC\n" \
                + "PCM\n" \
        ...
        message = paymul.Message(reference='CRLVPLIL',
                               dt=datetime.datetime(2004, 11, 11))
        message.batches.append(batch)
