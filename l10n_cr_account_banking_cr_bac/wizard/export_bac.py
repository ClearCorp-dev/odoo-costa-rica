...
class banking_export_bac_wizard(osv.osv_memory):
    _name = 'banking.export.bac.wizard'
    _description = 'BAC Export'
    _columns = {
    ...
    logger = logging.getLogger('export_bac')
    ...
    def create(self, cursor, uid, wizard_data, context=None):
        ...
        return super(banking_export_bac_wizard, self).create(
    ...
    def _create_account(self, oe_account, origin_country=None, is_origin_account=False):
        ...
        if oe_account.state == 'iban':
        ...
        elif oe_account.country_id.code == 'CR':
            self.logger.info('CR: %s %s' % (oe_account.country_id.code,oe_account.acc_number))
            ...
            if len(split) == 2:
                ...
                raise osv.except_osv(
                    _('Error'),
                    "Invalid CR acccount number '%s'" % oe_account.acc_number)
            paymul_account = paymul.CRAccount(
                number=accountno,
                sortcode=sortcode,
                holder=holder,
                currency=currency,
    ...
    def _create_transaction(self, line):
        ...
        if means is None:
            raise osv.except_osv('Error', "Invalid payment type mode for BAC '%s'" % line.order_id.mode.type.name)
    ...
    def wizard_export(self, cursor, uid, wizard_data_ids, context):
        '''
        Wizard to actually create the BAC file
        '''

        wizard_data = self.browse(cursor, uid, wizard_data_ids, context)[0]
        result_model = self.pool.get('banking.export.bac')
        ...
        if not isinstance(src_account, paymul.CRAccount):
            raise osv.except_osv(
                _('Error'),
        ...
        try:
            ...
            for po in payment_orders:
                ...
                bac_clientid = po.bac_clientid_id.clientid
        ...
        interchange = paymul.Interchange(client_id=bac_clientid,
                                         reference=ref,
                                         message=message)
