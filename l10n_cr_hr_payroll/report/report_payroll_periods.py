...
class PayrollReportByPeriods(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        super(PayrollReportByPeriods, self).__init__(cr, uid, name, context=context)
            ...
            for line in payslip.worked_days_line_ids:
                if line.code == code:
                    if payslip.credit_note:
                        total -= line.number_of_hours + number_of_hours * 8.0 #normal schedule in Costa Rica
                    else:
                        total += line.number_of_hours + number_of_hours * 8.0
        return total
